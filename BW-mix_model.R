...
predict_tree <- function(file){
  ...
  # Baum-Welch
  warn=has_warning(bw <- train(hmm,seq,method = "BaumWelch",logspace = FALSE,quiet=TRUE))
  if (warn==TRUE){
    print("model2")
    residues <- c("a","b","c")
    seq[data$isConstant==1]='c'
    E=matrix(c(.8,.1,.1,.1,.8,.1),nrow=2,ncol=3,byrow = TRUE)
    dimnames(E) <- list(states = states[-1], residues = residues)
    hmm <- structure(list(A = A, E = E), class = "HMM")
    warn=has_warning(bw <- train(hmm,seq,method = "BaumWelch",logspace = FALSE,quiet=TRUE))
  }
  if (warn==TRUE){
    ...
    seq[data$isConstant==1]='c'
    E=matrix(c(.7,.1,.1,.1,.1,.7,.1,.1),nrow=2,ncol=4,byrow = TRUE)
    dimnames(E) <- list(states = states[-1], residues = residues)
    hmm <- structure(list(A = A, E = E), class = "HMM")
    warn=has_warning(bw <- train(hmm,seq,method = "BaumWelch",logspace = FALSE,quiet=TRUE))
  }
